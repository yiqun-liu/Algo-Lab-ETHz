...
void testcase()
{
    ...
    // Explore private networks, keep only the shortest edges in final network
    for (int i = 0; i < nSpecies; i++)
    {
        ...
        for (int j = 0; j < mst.size(); j++)
        {
            ...
            boost::tie(oe_beg, oe_end) = boost::out_edges(v1, G);
            bool exist = false;
            while (oe_beg != oe_end)
            {
                if (v2 == boost::target(*oe_beg, G))
                {
                    exist = true;
                    if ( weight < minWeights[*oe_beg] )
                        minWeights[*oe_beg] = weight;
                }
                ++oe_beg;
            }
            if (!exist)
